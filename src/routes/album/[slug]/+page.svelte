...
<div class="bg-[var(--color-bg-secondary)]/80 backdrop-blur-xl border border-[var(--color-border)] rounded-2xl p-8 max-w-md w-full">
...
{#if form?.error}
<div class="bg-red-500/10 border border-red-500 text-red-400 px-4 py-3 rounded-lg text-sm mb-4">
{form.error}
</div>
{/if}
...
<button class="btn btn-primary text-sm" onclick={downloadSelected} disabled={isDownloading}>
{isDownloading ? 'Downloading...' : `Download (${selectedPhotos.size})`}
</button>
{:else if !isSelecting}
<button class="btn btn-primary text-sm" onclick={downloadAlbum} disabled={isDownloading}>
...
<button class="text-sm font-medium opacity-90 hover:opacity-100" onclick={selectAll}>Select All</button>
<button class="text-sm font-medium opacity-90 hover:opacity-100" onclick={clearSelection}>Clear</button>
</div>
</div>
</div>
{/if}
...
class="px-3 py-1.5 rounded-full text-sm transition-colors {!data.selectedTag ? 'bg-blue-500 text-white' : 'bg-[var(--color-bg-tertiary)] text-gray-300 hover:bg-[var(--color-border)]'}"
>
All
</a>
...
class="px-3 py-1.5 rounded-full text-sm transition-colors {data.selectedTag === tag.slug ? 'bg-blue-500 text-white' : 'bg-[var(--color-bg-tertiary)] text-gray-300 hover:bg-[var(--color-border)]'}"
>
{tag.name}
</a>
{/each}
...
loading="lazy"
class="w-full {data.album.layout === 'masonry' ? 'h-auto' : 'aspect-square object-cover'}"
/>
{#if isSelecting}
<div class="absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center transition-colors {selectedPhotos.has(photo.id) ? 'bg-blue-500 text-white' : 'bg-white/90 border-2 border-gray-300'}">
